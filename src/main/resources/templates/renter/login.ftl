...
<head>
    ...
    <script src="${base}/static/js/renter/common.js"></script>
</head>
<script>
    if(undefined != tokenId && tokenId != null && tokenId != 'null'){
        window.location.href = "${base}/renter/goHome/" + userId + "?tokenId=" + tokenId;
    }
</script>
<body>
    ...
    layui.use(['element', 'layer', 'table', 'form'], function () {
        ...
        var login = function () {
            ...
            jhxhr.done(function (res) {
                if(res.code == 1){
                    $.cookie("rent_renter_tokenId",res.data.tokenId,{expires: 7, path: '/rent'});
                    $.cookie("rent_renter_userId",res.data.userId,{expires: 7, path: '/rent'});
                    window.location.href = requestBaseUrl +  "/renter/goHome/" + res.data.userId + "?tokenId=" + res.data.tokenId;
                }else{
